...
struct ChoresTasksSettingsView: View {
    ...
    var filteredApps: [TaskApplication] {
            ApplicationData.appList.filter { app in
                app.availableTasks.contains { $0.type == .chores }
            }
        }
        ...
        VStack {
            ...
            VStack(alignment: .leading) {
                ...
                if let app = selectedApp {
                    Picker("Select Task", selection: $selectedTaskOption) {
                        ForEach(app.availableTasks.filter { $0.type == .chores }, id: \.name) { task in
            ...
            Button("Save") {
                ...
                let newTask = DailyTask(
                    ...
                    type: .chores, // Change this if needed for different task types
...
struct ChoresTasksSettingsView_Previews: PreviewProvider {
    @State static var isPresented = true

    static var previews: some View {
        ChoresTasksSettingsView(
