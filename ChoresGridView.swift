...
struct ChoresGridView: View {
    ...
    var choresTasks: [DailyTask] {
        taskListModel.dailyTasks.filter { $0.type == .chores }
    }
...
#Preview {
    ChoresGridView(taskListModel: TaskListModel.shared, onTaskDelete: { task in TaskListModel.shared.deleteTask(task) })
}
