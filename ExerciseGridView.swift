...
struct ExerciseGridView: View {
    ...
    var exerciseTasks: [DailyTask] {
        taskListModel.dailyTasks.filter { $0.type == .exercise }
    }
...
#Preview {
    ExerciseGridView(taskListModel: TaskListModel.shared, onTaskDelete: { task in TaskListModel.shared.deleteTask(task) })
}
