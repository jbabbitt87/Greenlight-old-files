...
struct DailyTasksSettingsView: View {
            ...
            VStack(alignment: .leading) {
                ...
                Picker("Select App", selection: $selectedApplication) {
                    ForEach(ApplicationData.appList.filter { app in
                        app.availableTasks.contains { $0.type == .appSpecific }
                    }, id: \.name) { app in
            ...
            VStack(alignment: .leading) {
                ...
                if let app = selectedApp {
                    Picker("Select Task", selection: $selectedTaskOption) {
                        ForEach(app.availableTasks.filter { $0.type == .appSpecific }, id: \.name) { task in
...
struct DailyTasksSettingsView_Previews: PreviewProvider {
